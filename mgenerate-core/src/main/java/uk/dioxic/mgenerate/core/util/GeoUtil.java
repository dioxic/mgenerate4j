...
public final class GeoUtil {
     ...
     */
    private static void reverse(final Coordinates[] x, final int from, final int to) {
        for (int i = from, j = to; i < j; i++, j--) {
            final Coordinates tmp = x[i];
            x[i] = x[j];
            x[j] = tmp;
        }
    }
     ...
     */
    private static double moveCost(final Coordinates a, final Coordinates b,
        ...
        return (MathUtil.sqrt(_ac) + MathUtil.sqrt(_bd)) -
                (MathUtil.sqrt(_ab) + MathUtil.sqrt(_cd));
    }

    /**
     * set active bits for 4 vertices making up edges ab, cd.
     */
    private static void activate(final Coordinates a, final Coordinates b,
                          final Coordinates c, final Coordinates d) {
        a.setActive(true);
        b.setActive(true);
        c.setActive(true);
     ...
     */
    private static double findMove(final int current, final Coordinates currentPoint,
                            final Coordinates[] points, final int numPoints) {

        // previous and next city index and point object.
        final int prev = wrap(current - 1, numPoints);
        final int next = wrap(current + 1, numPoints);
        final Coordinates prevPoint = points[prev];
        final Coordinates nextPoint = points[next];
        ...
        for (int i = wrap(current + 2, numPoints), j = wrap(current + 3, numPoints);
             j != current;
             i = j, j = wrap(j + 1, numPoints)) {

            final Coordinates c = points[i];
            final Coordinates d = points[j];
     ...
     */
    public static void optimise(final Coordinates[] points) {
        ...
        // numPoints - visited = total number of active points.
        // current = current point being explored.
        int visited = 0, current = 0;

        // terminate when a full rotation of of static order from point 1:N
        ...
        while (visited < numPoints) {
            final Coordinates currentPoint = points[current];
            if (currentPoint.isActive()) {
                ...
                // if a move was found, go to previous point.
                // best is += modified delta.
                if (modified < 0) {
                    current = wrap(current - 1, numPoints);
                    visited = 0;
                    continue;
                }
            ...
            // if point is inactive or no moves found, go to next point.
            current = wrap(current + 1, numPoints);
            visited++;
        }
    }
